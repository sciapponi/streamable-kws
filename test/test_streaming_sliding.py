...
def test_onnx_streaming(
    ...
    n_fft=400,
    hop_length=160,
    chunk_size_ms=200,  # Fixed window size of 200ms
    use_running_stats=False,
    alpha=0.95
):
    """
    Test an ONNX model in streaming mode by processing sliding windows of audio
    with 200ms chunks every 20ms.
    """
    ...
    step_size_samples = int(sample_rate * 50 / 1000)  # 20ms step size
    print(f"Processing in sliding windows: chunk size {chunk_size_ms}ms every 20ms (step size: {step_size_samples} samples)")
    ...
    for chunk_start in tqdm(range(0, len(audio) - chunk_size_samples + 1, step_size_samples), desc="Processing chunks"):
        ...
        # For plotting, we'll associate this prediction with all frames in the step (20ms)
    ...
    if len(all_predictions_raw_vectors) > 0 and num_classes > 0:
        ...
        plt.title("Predictions Over Time (Window Centers)"); plt.xlabel("Time (s)")
    ...
    output_filename = f"sliding_window_predictions_{stats_type}_{chunk_size_ms}ms_step20ms_alpha{alpha_str}.png"
